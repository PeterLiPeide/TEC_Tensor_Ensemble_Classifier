...
Df = loadmat('C:\\Users\\kira2\\Desktop\\TEC_Full_Code\\Simu_Df_T1.mat')
X1 = Df['Class_1']
X2 = Df['Class_2']
...
for i in range(100):
    ...
    m1 = STM.RPSTM(Xtrain, ytrain, num_rank=3, P = [45, 45, 45], Decomposition=False, D = [50, 50, 50])
    m2 = STM.RPSTM(Xtrain, ytrain, num_rank=3, P = [45, 45, 45], loss_type="SQHinge", solver='Gaussian', Decomposition=False, D = [50, 50, 50])
    m3 = STM.TEC(Xtrain, ytrain, P = [45, 45, 45], num_rank=3, Random_Projection=True, num_ensemble=3, Decomposition=False, D = [50, 50, 50])
    m4 = STM.TEC(Xtrain, ytrain, P = [45, 45, 45], num_rank=3, Random_Projection=True, loss_type='SQHinge', solver='Gaussian', num_ensemble=3, Decomposition=False, D = [50, 50, 50])
    m1.fit('Gaussian', [0.5, 0.5, 0.5], 1 / 160)
    m2.fit('Gaussian', [0.5, 0.5, 0.5], 0.5)  # Notice the C parameter has different meaning in SQHinge and Hinge Solver
    m3.fit('Gaussian', [0.5, 0.5, 0.5], 1 / 160)
    m4.fit('Gaussian', [0.5, 0.5, 0.5], 0.5)
...
df.to_csv('T1_Result.csv', index=False)
# %%
